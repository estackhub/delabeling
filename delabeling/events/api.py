...
def whitelabel_patch():
	...
	update_field_label()
	if cint(get_frappe_version()) >= 13 and not frappe.db.get_single_value('Delabeling Setting', 'ignore_onboard_labeling'):
...
def boot_session(bootinfo):
	...
	if frappe.session['user']!='Guest':
		...
		bootinfo.whitelabel_setting = frappe.get_doc("Delabeling Setting","Delabeling Setting")

@frappe.whitelist()
def ignore_update_popup():
	if not frappe.db.get_single_value('Delabeling Setting', 'disable_new_update_popup'):
